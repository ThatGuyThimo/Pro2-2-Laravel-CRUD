...
<body class="bg-gray-500">
    ...
    <main id="content" class="flex flex-row">
        ...
        <div class="m-2 bg-white rounded border-2 border-red-500 p-2">
            <p class="text-2xl">{{$value['title']}}</p>
            <p class="text-sm">{{$value['category']}}</p>
            <p>{{$value['content']}}</p>
            <a href="{{ route('details', ['id' => $value['id']] ) }}"><button class="p-2 m-1 transition-all duration-300 bg-blue-500 hover:bg-blue-700 rounded text-white">Details</button></a>
            ...
            @if ($value['status'] == 0)
                <a href="{{ route('togglepost', ['id' => $value['id']]) }}"><button class="p-2 m-1 transition-all duration-300 bg-blue-500 hover:bg-blue-700 rounded text-white">disable</button></a>
            @else
            <a href="{{ route('togglepost', ['id' => $value['id']]) }}"><button class="p-2 m-1 transition-all duration-300 bg-blue-500 hover:bg-blue-700 rounded text-white">enable</button></a>
            @endif
        </div>
        @endif
        @endforeach
